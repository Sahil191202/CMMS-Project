  ...
  },
  {
    ...
    render: (val) => (
      <div className="flex items-center gap-1.5">
        <span
          className={`w-2 h-2 rounded-full shrink-0 ${PRIORITY_DOT[val] || "bg-gray-400"}`}
        />
...
const FilterBar = ({
  ...
  const btOptions = (breakdownTypes || []).map((bt) => ({
    value: bt.id,
    label: bt.name,
  }));
    ...
    <div className="flex flex-col gap-5">
      ...
      {tickets.length === 0 ? (
        <EmptyState
          ...
          message="Try changing filters or create a new ticket."
        />
      ) : (
        <DataTable
          columns={columns}

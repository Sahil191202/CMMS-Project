...
const KPICard = ({
  ...
  return (
    <div
      className={`bg-gray-900 border border-gray-800 p-5 flex flex-col gap-3 relative overflow-hidden group hover:border-gray-700 transition-colors duration-200 ${className}`}
    >
      {/* Top row: title + icon */}
      <div className="flex items-start justify-between">
        ...
        {icon && (
          <span className="text-amber-500/60 group-hover:text-amber-500 transition-colors duration-200">
            {icon}
          </span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-3xl font-black text-gray-100 tracking-tight leading-none">
            {value ?? '—'}
          </span>
          {unit && (
